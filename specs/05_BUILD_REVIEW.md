...
- P0-P3 conformance hardening: production desktop builds exclude manual capture/job-processing diagnostics; Windows capture preserves native PNG dimensions; analysis indexes bounded adjacent OCR chunks with overlap; search chunks persist inclusive OCR block ranges and citations return all covered OCR bounds; analysis renew/complete/fail operations require the current lease owner and expiry; and FTS term extraction preserves Unicode alphanumerics for the `unicode61` tokenizer.
...
The 2026-06-22 P0-P3 conformance hardening pass re-ran `cargo fmt --check`, `cargo clippy --workspace --all-targets -- -D warnings`, `cargo test --workspace --quiet`, `npm ci`, `npm run lint`, and `npm run build`. Regression coverage now includes bounded/overlapping OCR chunk construction, migration `0008` for inclusive chunk ranges, multi-bound citation recovery, stale-lease rejection, Unicode FTS term preservation, and native-dimension PNG encoding. This pass does not close item 15 or item 18.
